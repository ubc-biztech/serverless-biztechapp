...
export const del = protect(Access.ADMIN, async (event) => {
  try {
...
export const update = protect(Access.ADMIN, async (event) => {
  try {
    const { id, year } = validateEventPath(event.pathParameters ?? null);
...
export const createThumbnailPicUploadUrl = protect(Access.ADMIN, async (event) => {
  try {
...
export const qaCreate = protect(Access.USER, async (event) => {
  try {
...
export const qaUpvote = protect(Access.USER, async (event) => {
  try {
    const email = event.auth!.email;
...
export const qaPatch = protect(Access.ADMIN, async (event) => {
  try {
    const email = event.auth!.email;
