...
/** Escapes Slack mrkdwn — an unescaped question body can render as a link. */
...
export function buildQaSlackMessage({
  ...
  return {
    ...
    text: `New Q&A question for ${escapeSlackText(eventName)}`,
    blocks: [
      {
        type: "section",
        text: {
      ...
      },
      {
        type: "context",
        elements: [
          { type: "mrkdwn", text: `Category: *${escapeSlackText(category)}*` },
        ],
      },
    ],
  };
