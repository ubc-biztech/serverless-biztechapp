...
export const postBuy = protect(Access.USER, async (event, ctx, callback) => {
  try {
...
export const postSell = protect(Access.USER, async (event, ctx, callback) => {
  try {
    const userId = event.auth.email;
...
export const getPortfolio = protect(Access.USER, async (event, ctx, callback) => {
  try {
    const userId = event.auth.email;
