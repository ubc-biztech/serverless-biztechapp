		...
		return this.checkValidQR(qrCodeID, eventIDAndYear).then(async (qr) => {
			if (qr === null) {
				throw helpers.createResponse(403, {
					message:
						'Invalid QR code - not scannable for this BizTech event!',
					data: data,
				});
			...
			return await docClient
				...
				.then(async (result) => {
					...
					const updateParams = {
						...
						UpdateExpression:
							'set points = :points, scannedQRs = :scannedQRs',
						ExpressionAttributeValues: {
							':points': userRegistration.points,
							':scannedQRs': JSON.stringify(
								scannedQRs.concat(qrCodeID)
							),
						},
