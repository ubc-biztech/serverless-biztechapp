...
export const ensureAuthenticatedUser = protect(Access.USER, async (event) => {
...
export const update = protect(Access.USER, async (event) => {
  try {
    ...
    // Only the owner (or an admin) may modify this record.
    if (!event.auth.isAdmin && email.toLowerCase() !== event.auth.email)
...
export const getAll = protect(Access.ADMIN, async () => {
  try {
...
export const favouriteEvent = protect(Access.USER, async (event) => {
  try {
    // Only the owner (or an admin) may modify this record.
    const targetEmail = event.pathParameters?.email;
    if (!event.auth.isAdmin && targetEmail?.toLowerCase() !== event.auth.email)
      return helpers.createResponse(403, {
        message: "You can only modify your own favourites"
      });
...
export const del = protect(Access.USER, async (event) => {
  try {
    ...
    // Only the owner (or an admin) may delete this record.
    if (!event.auth.isAdmin && email.toLowerCase() !== event.auth.email)
      return helpers.createResponse(403, {
