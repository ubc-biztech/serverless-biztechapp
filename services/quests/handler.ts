...
export const updateQuest = protect(Access.USER, async (event) => {
  try {
    if (
...
export const getQuest = protect(Access.USER, async (event) => {
  try {
    if (
      !event.pathParameters ||
      !event.pathParameters.event_id ||
...
export const getQuestsByEvent = protect(Access.ADMIN, async (event) => {
  try {
    if (
      !event.pathParameters ||
      !event.pathParameters.event_id ||
