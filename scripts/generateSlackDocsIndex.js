...
async function main() {
  ...
  const jsonOutputArg = parseArg("json-out", "");
  const jsonOutputPath = jsonOutputArg ? path.resolve(jsonOutputArg) : "";
  ...
  const payload = {
    docsBaseUrl,
    docsIndexGeneratedAt: generatedAt,
...
export const docsBaseUrl = ${JSON.stringify(payload.docsBaseUrl)};
export const docsIndexGeneratedAt = ${JSON.stringify(payload.docsIndexGeneratedAt)};
export const docsChunkCount = ${payload.docsChunkCount};
export const docsChunks = ${JSON.stringify(payload.docsChunks)};
`;
  ...
  if (jsonOutputPath) {
    ...
    await fs.writeFile(jsonOutputPath, JSON.stringify(payload), "utf8");
  }
  ...
  if (jsonOutputPath) {
    console.log(`Wrote docs index JSON -> ${jsonOutputPath}`);
  }
}
