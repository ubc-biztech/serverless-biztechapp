...
const generateNFCsForAllRegistrations = async (eventID, year) => {
    const registrations = await client.send(new ScanCommand({
        TableName: "biztechRegistrations" + (process.env.ENVIRONMENT || ""),
        FilterExpression: "#eventIDYear = :eventIDYear",
...
generateNFCsForAllRegistrations("blueprint", 2025);
